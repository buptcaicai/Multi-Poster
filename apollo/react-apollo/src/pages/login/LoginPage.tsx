...
export default function LoginForm() {
   ...
   const [login, { data, loading, error }] = useMutation<Response>(PASSWORD_LOGIN);
   ...
   const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (response.data?.passwordLogin) {
         setUser && setUser(response.data.passwordLogin);
         navigate('/');
      } else {
   ...
   return <div className="h-screen flex flex-col items-center justify-center">
      ...
      <div className="w-[100%] justify-center flex mt-9">
         <form className="w-[70%] h-[70%] p-3 max-w-150" method="post" onSubmit={submitHandler}>
            <p>
            ...
            <div className="flex items-center justify-end">
               {(error || data && (!data.passwordLogin || data.passwordLogin.roles.length === 0)) && (<span className="text-red-500 text-xl font-medium text-center">Login Fail {error && <p>{error.message}</p>}</span>)}
               <button type="submit" className="flex items-center justify-center ml-3 mr-3 p-3 m-2 bg-purple-950 text-white text-2xl 
               font-bold hover:bg-transparent hover:cursor-pointer rounded-2xl w-22 h-12">
                  {loading ? (<div role="status" className="leading-5">
                     <svg aria-hidden="true" className="inline w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 
